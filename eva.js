...
function getEvents() {
	...
	dbConn.query(SQL +mo +' and day(event) = ' +day +' ORDER BY lname', (err, result, fields) => {
		if (err) {
			Log.write('getEvents Query error: ' +err.message, timeStamp);
			throw err;
		};
//		console.log(result);
		...
		msg+= '</table>';
		msg+= os.hostname() +';  ' +os.platform() +';  ' +os.release() +';  ' +(os.uptime() /24 /3600).toFixed(2) +"<br />\n"
//		console.log(msg);
		...
		//	Conns.splice(Conns.indexOf(key), 1); //remove item
			if (key.slice(0, 3) != 'mon')
				msg+= '<tr style="background-color:#ddf"><td>' +key +'</td><td>' +Uptimes[key] +" days</td></tr>\n";
		});
        msg+= '</table>';
		const mailParm= {
...
//			to: '8646307343@tmomail.net',
		...
		mailConn.sendMail(mailParm, (err, info) => {
			if (err) {
				Log.write('sendMail: ' +err.message, timeStamp);
				throw err;
			};
...
MQ.client.on('connect', () => {	
	...
	MQ.client.subscribe('+/conn',{qos:1});
	MQ.client.subscribe('+/told/uptime',{qos:1});
...
MQ.client.on('message',(topic, payload) => {
	if (topic == cfg.clientID +'/get/dev') MQ.pub('/told/dev', cfg.loc +'/events',1,false);
	else if (topic == cfg.clientID +'/get/uptime') MQ.pub('/told/uptime', ((Date.now() -startTime) /24 /3600000).toFixed(2),1,false );
	else if (topic == cfg.clientID +'/get/loc') MQ.pub('/told/loc', os.hostname() +'; ' +os.platform() +'; ' +os.release() +'; ' +(os.uptime() /24 /3600).toFixed(2) +'; ' +((Date.now() -startTime) /24 /3600).toFixed(2),1,false);
...
setTimeout(() => {
	Object.keys(Conns).forEach(key => {
		if (Conns[key] == 'ready') MQ.pub(key +'/get/loc');
		else Locs[key] = 'n/a';
		setTimeout(() => {console.log(key, Conns[key], Locs[key])}, 1000);
	});
}, 1000);
*/